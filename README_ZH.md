...
- **并发与路由**：支持多账号与并发，链路极致优化（超低的前置开销和交互损耗）
...
**切换模型** 可以为视觉（截图分析）、音频（语音问答）指定不同的模型，支持多选并发提问，左滑右滑切换 AI 回答
...
本项目在迭代开发中，如果您有**BUG 反馈、功能需求、任何使用疑问**，欢迎加入群组进行反馈。
如果您有**具体的功能需求或者 AI 服务需要适配**，欢迎与我沟通，评估后为您实现。
